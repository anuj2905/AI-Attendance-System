...
def student_dashboard():
    ...
    with c2:
        if st.button('Enroll in Subject', key="enroll_btn"):
            enroll_dialog()
...
def student_screen():
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go back to Home", key="back_home"):
    ...
    if show_registration:
        with st.container(border=True):
            ...
            try:
                audio_data = st.audio_input("Say: I am present")
            except:
